...
const GiderTipiUpdateModal = forwardRef<HTMLDivElement, IModalCreateitemProps>(
  ({ open, onOpenChange, item }, ref) => {
    ...
    useEffect(() => {
      if (item && item.Tanim) {
        setitemValue({
          Tanim: item.Tanim,
          GiderTipiID: Number(item.GiderTipiID)
        })
      }
    }, [item])

    const handleUpdateitem = async () => {
      if (!itemValue.Tanim || itemValue.GiderTipiID == 0) {
        setSuccessMessage('')
      ...
      try {
        ...
        const response: any = await axios.post(`${API_URL}/gider-tipleri/update`, itemValue, {
          headers: {
      ...
      <Dialog open={open} onOpenChange={() => onOpenChange('close')}>
        <DialogContent className="max-w-[1000px]" ref={ref}>
          <DialogHeader className="flex flex-col gap-2.5">
            <DialogTitle className="text-2xl font-bold">Gider Tipi ni Düzenle</DialogTitle>
            <DialogDescription>Gider Tipi hakkında bilgi verin</DialogDescription>
          </DialogHeader>
