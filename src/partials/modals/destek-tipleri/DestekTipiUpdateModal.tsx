...
const DestekTipiUpdateModal = forwardRef<HTMLDivElement, IModalCreateitemProps>(
    ...
    useEffect(() => {
      if (item && item.Tanim) {
        setitemValue({
          ...
          DestekTipiID: Number(item.DestekTipiID)
        })
      }
    }, [item])

    const handleUpdateitem = async () => {
      if (!itemValue.Tanim || itemValue.DestekTipiID == 0) {
        setSuccessMessage('')
      ...
      try {
        ...
        const response: any = await axios.post(`${API_URL}/destek-tipleri/update`, itemValue, {
          headers: {
      ...
      <Dialog open={open} onOpenChange={() => onOpenChange('close')}>
        <DialogContent className="max-w-[1000px]" ref={ref}>
          <DialogHeader className="flex flex-col gap-2.5">
            <DialogTitle className="text-2xl font-bold">Destek Tipi ni Düzenle</DialogTitle>
