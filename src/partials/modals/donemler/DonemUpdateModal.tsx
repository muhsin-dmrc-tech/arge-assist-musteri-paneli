...
const DonemUpdateModal = forwardRef<HTMLDivElement, IModalCreateitemProps>(
  ({ open, onOpenChange, item }, ref) => {
    ...
    useEffect(() => {
      if (item && item.DonemAdi) {
        setitemValue({
          DonemAdi: item.DonemAdi,
          DonemID: Number(item.DonemID),
          Ay: item.Ay,
          Yil: item.Yil
        })
      }
    ...
    const handleUpdateitem = async () => {
      if (!itemValue.DonemAdi || itemValue.DonemID == 0) {
        setSuccessMessage('')
      ...
      if (itemValue.DonemAdi.length < 2 || itemValue.DonemAdi.length > 255) {
        setSuccessMessage('')
        setError('Dönem adı en az 2 ve en fazla 255 karakter olabilir.');
        return
      }
      ...
      try {
        ...
        const response: any = await axios.post(`${API_URL}/donem/update`, itemValue, {
          headers: {
      ...
      <Dialog open={open} onOpenChange={() => onOpenChange('close')}>
        <DialogContent className="max-w-[1000px]" ref={ref}>
          <DialogHeader className="flex flex-col gap-2.5">
            <DialogTitle className="text-2xl font-bold">Dönem i Düzenle</DialogTitle>
