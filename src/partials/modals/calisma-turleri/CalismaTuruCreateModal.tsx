...
const CalismaTuruCreateModal = forwardRef<HTMLDivElement, IModalCreateitemProps>(
  ({ open, onOpenChange }, ref) => {
    ...
    const handleCreateitem = async () => {
      ...
      try {
        const response: any = await axios.post(`${API_URL}/calisma-turu/create`, itemValue, {
          headers: {
      ...
      <Dialog open={open} onOpenChange={() => onOpenChange('close')}>
        <DialogContent className="max-w-[1000px]" ref={ref}>
          <DialogHeader className="flex flex-col gap-2.5">
            <DialogTitle className="text-2xl font-bold">Yeni Çalisma Türü Ekle</DialogTitle>
