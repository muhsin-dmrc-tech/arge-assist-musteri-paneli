...
const CalismaTuruUpdateModal = forwardRef<HTMLDivElement, IModalCreateitemProps>(
  ({ open, onOpenChange, item }, ref) => {
    ...
    useEffect(() => {
      if (item && item.Tanim) {
        setitemValue({
          Tanim: item.Tanim,
          CalismaTuruID: Number(item.CalismaTuruID)
        })
      }
    }, [item])

    const handleUpdateitem = async () => {
      if (!itemValue.Tanim || itemValue.CalismaTuruID == 0) {
        setSuccessMessage('')
      ...
      try {
        ...
        const response: any = await axios.post(`${API_URL}/calisma-turu/update`, itemValue, {
          headers: {
      ...
      <Dialog open={open} onOpenChange={() => onOpenChange('close')}>
        <DialogContent className="max-w-[1000px]" ref={ref}>
          <DialogHeader className="flex flex-col gap-2.5">
            <DialogTitle className="text-2xl font-bold">Çalisma Türü nü Düzenle</DialogTitle>
            <DialogDescription>Çalisma türü hakkında bilgi verin</DialogDescription>
          </DialogHeader>
