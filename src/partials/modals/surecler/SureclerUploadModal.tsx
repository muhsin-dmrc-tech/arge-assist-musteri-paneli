...
const SureclerUploadModal = forwardRef<HTMLDivElement, IModalCreateitemProps>(
  ({ open, onOpenChange, item }, ref) => {
    ...
    useEffect(() => {
      if (item && item.SurecAdi) {
        setitemValue({
          ID: item.ID,
          SurecAdi: item.SurecAdi,
    ...
    const handleUpdateitem = async () => {
      if (!itemValue.SurecAdi || !itemValue.Anahtar) {
        setSuccessMessage('')
      ...
      if (itemValue.SurecAdi.length < 2 || itemValue.SurecAdi.length > 255) {
        setSuccessMessage('')
        setError('Süreç Adı en az 2 ve en fazla 255 karakter olabilir.');
        return
      }
      if (itemValue.Anahtar.length < 2 || itemValue.Anahtar.length > 255) {
        setSuccessMessage('')
        setError('Anahtar en az 2 ve en fazla 255 karakter olabilir.');
        return
      }
      ...
      try {
        ...
        const response: any = await axios.post(`${API_URL}/surecler/upload`, itemValue, {
          headers: {
      ...
      <Dialog open={open} onOpenChange={() => onOpenChange('close')}>
        <DialogContent className="max-w-[1000px]" ref={ref}>
          <DialogHeader className="flex flex-col gap-2.5">
            <DialogTitle className="text-2xl font-bold">{item ? 'Süreci Düzenle' : 'Süreç Ekle'}</DialogTitle>
            <DialogDescription>Süreç hakkında bilgi verin</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-2.5 p-2.5">
            ...
            <div className="flex flex-col gap-1">
              <InputLabel required>Süreç Adı</InputLabel>
              <Input className="w-full"
                value={itemValue.SurecAdi}
                required
                minLength={2}
                maxLength={255}
                placeholder="Süreç Adı"
                onChange={(e) => setitemValue({ ...itemValue, SurecAdi: e.target.value })} />
            </div>
