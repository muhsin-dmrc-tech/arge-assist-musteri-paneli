...
const MuafiyetTipiUpdateModal = forwardRef<HTMLDivElement, IModalCreateitemProps>(
  ({ open, onOpenChange, item }, ref) => {
    ...
    useEffect(() => {
      if (item && item.Tanim) {
        setitemValue({
          Tanim: item.Tanim,
          MuafiyetTipiID: Number(item.MuafiyetTipiID)
        })
      }
    }, [item])

    const handleUpdateitem = async () => {
      if (!itemValue.Tanim || itemValue.MuafiyetTipiID == 0) {
        setSuccessMessage('')
      ...
      try {
        ...
        const response: any = await axios.post(`${API_URL}/muafiyet-tipleri/update`, itemValue, {
          headers: {
      ...
      <Dialog open={open} onOpenChange={() => onOpenChange('close')}>
        <DialogContent className="max-w-[1000px]" ref={ref}>
          <DialogHeader className="flex flex-col gap-2.5">
            <DialogTitle className="text-2xl font-bold">Muafiyet Tipi ni Düzenle</DialogTitle>
            <DialogDescription>Muafiyet Tipi hakkında bilgi verin</DialogDescription>
          </DialogHeader>
...
export { MuafiyetTipiUpdateModal };
