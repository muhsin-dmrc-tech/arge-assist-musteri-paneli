...
const AbonelikPlanlariUpdateModal = forwardRef<HTMLDivElement, IModalCreateitemProps>(
  ({ open, onOpenChange, item }, ref) => {
    ...
    useEffect(() => {
      if (item && item.AbonelikPlanID) {
        setitemValue({
          PlanAdi: item.PlanAdi,
          Aciklama: item.Aciklama,
          Fiyat: item.Fiyat,
          AbonelikPlanID: Number(item.AbonelikPlanID)
        })
      }
    }, [item])
    ...
    const handleUpdateitem = async () => {
      ...
      if (!itemValue.PlanAdi || !itemValue.Aciklama || !itemValue.Fiyat || itemValue.AbonelikPlanID === 0 || itemValue.AbonelikPlanID === undefined) {
        setSuccessMessage('')
        setError('Plan Adı, Abonelik Plan ID, Açıklama ve Fiyat zorunludur');
      ...
      try {
        ...
        const response: any = await axios.post(`${API_URL}/abonelik-planlari/update`, itemValue, {
          headers: {
      ...
      <Dialog open={open} onOpenChange={() => onOpenChange('close')}>
        <DialogContent className="max-w-[1000px]" ref={ref}>
          <DialogHeader className="flex flex-col gap-2.5">
            <DialogTitle className="text-2xl font-bold">Abonelik Planı nı Düzenle</DialogTitle>
...
export { AbonelikPlanlariUpdateModal };
