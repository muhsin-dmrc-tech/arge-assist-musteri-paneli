...
const Step2b = ({ fetchFile, setErrors, errors, tamamlananlar, setTamamlananlar, adimSelect, handleSubmit, goToNext }: Step1Props) => {
    ...
    useEffect(() => {
        const processExistingFile = async () => {
            if (projeRaporu?.OnayliMuhtasarVePrim && typeof projeRaporu.OnayliMuhtasarVePrim === 'string' && !dosyaEklendi) {
                resetStatesFunc()
                try {
                    setLoading(true);
                    ...
                    if (response.data.error) {
                        ...
                        if (response.data?.muhtasar && response.data?.muhtasar.muhtasarMeta) {
                            setOnayliMuhtasarMeta(response.data?.muhtasar.muhtasarMeta)
                        }
                        ...
                        if (response.data?.muhtasar?.personelListesi && response.data?.muhtasar?.personelListesi?.farklilar && response.data?.muhtasar?.personelListesi?.farklilar?.length > 0) {
                            //setGeciciListe(response.data?.muhtasar?.personelListesi.geciciListe ?? [])
                            //setFarkliListesi(response.data?.muhtasar?.personelListesi.farklilar ?? [])
                            setOnayliMuhtasarTableData(response.data?.muhtasar?.personelListesi.geciciListe ?? [])

                            if (response.data?.sgkHizmet && response.data?.sgkHizmet?.geciciListe) {
                                setOnayliSgkHizmetListesi(response.data.sgkHizmet.geciciListe ?? [])
        ...
        processExistingFile();
    }, [projeRaporu?.OnayliMuhtasarVePrim, dosyaEklendi]);
    ...
    const handleSubmitFunc = async (file: File, checkedList?: any) => {
        ...
        if (response.error) {
            ...
            if (response?.muhtasar && response?.muhtasar.muhtasarMeta) {
                setOnayliMuhtasarMeta(response?.muhtasar.muhtasarMeta)
            } else {
            ...
            if (response?.personelListesi && response?.personelListesi?.personelListesi && response?.personelListesi?.personelListesi?.farklilar && response?.personelListesi?.personelListesi?.farklilar?.length > 0) {
                ...
                setOnayliMuhtasarTableData(response?.personelListesi?.personelListesi?.geciciListe ?? [])
                setGeciciListe([])
                setFarkliListesi([])
                setErrors(prev => prev.filter(error => error.index !== 1));
                setTamamlananlar(prev =>
            ...
            if (response?.personelListesi?.muhtasarMeta) {
                setOnayliMuhtasarMeta(response?.personelListesi?.muhtasarMeta)
            } else {
                setOnayliMuhtasarMeta({
                    vergiKesintiTutari: '',
                    terkinTutari: '',
    ...
    const resetStatesFunc = () => {
        ...
        setOnayliMuhtasarMeta({} as MuhtasarBeyannameMeta);
    ...
    const aktifListe = onayliMuhtasarTableData.length > 0 ? onayliMuhtasarTableData : geciciListe;
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {aktifListe.length > 0 && <Box sx={{ mt: 3 }}>
                <Typography><strong>Proje Kodu:</strong> {onayliMuhtasarMeta?.projeKodu}</Typography>
                <Typography><strong>Vergi Kesinti Tutarı:</strong> {onayliMuhtasarMeta?.vergiKesintiTutari}</Typography>
                <Typography><strong>4691 Sayılı Kanun Gereği Terkin Edilen Tutar:</strong> {onayliMuhtasarMeta?.terkinTutari}</Typography>
            </Box>}
