...
const Step1a = ({ fetchFile, setErrors, errors, tamamlananlar, handleSubmit, setTamamlananlar, adimSelect, goToNext }: Step1Props) => {
    ...
    useEffect(() => {
        const processExistingFile = async () => {
            if (projeRaporu?.SGKHizmet && typeof projeRaporu.SGKHizmet === 'string' && !dosyaEklendi && !projeRaporu?.MuhtasarVePrim) {
                resetStatesFunc()
                try {
                    setLoading(true);
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        ...
        if (response.error) {
            ...
            setErrors(prev => prev.filter(error => error.index !== 0));
            setSgkHizmetListesi(response.personelListesi?.geciciListe ?? [])
            setTamamlananlar(prev =>
                produce(prev, draft => {
                    const exists = draft.some(item => item.index === 0);
                    if (!exists) {
    ...
    const aktifListe = sgkHizmetListesi.length > 0 ? sgkHizmetListesi : [];
