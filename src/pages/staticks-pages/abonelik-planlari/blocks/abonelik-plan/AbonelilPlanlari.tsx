...
import { AbonelikPlanlariCreateModal, AbonelikPlanlariUpdateModal } from '@/partials/modals/abonelik-planlari';
...
const AbonelikPlanlari = () => {
  ...
  const fetchBusinessTypess = async (queryParams: any) => {
    try {
      const response = await axios.get(
        `${API_URL}/abonelik-planlari/get-planlar?${queryParams.toString()}`
      );
  ...
  const columns = useMemo<ColumnDef<IITemsTypesData>[]>(
    () => [
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.PlanAdi,
        id: 'PlanAdi',
        header: ({ column }) => <DataGridColumnHeader title="Plan Adı" filter={<ColumnInputFilter column={column} />} column={column} />,
        enableSorting: true,
        cell: ({ row }) => {
          return (
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.Fiyat,
        id: 'Fiyat',
        header: ({ column }) => <DataGridColumnHeader title="Fiyat(TL)" column={column} />,
        enableSorting: false,
        cell: ({ row }) => {
          return (
            <div className="flex items-center gap-4">
      ...
      },
      {
        ...
        enableSorting: false,
        cell: ({ row }) => {
          return activeLoading[row.original.AbonelikPlanID] ? (
            ...
            <div className="switch switch-sm">
              <input
                type="checkbox"
                name="param"
                checked={row.original.Aktifmi}
                onChange={async (e) => {
                  await isActiveFunc(row.original.AbonelikPlanID, e.target.checked);
                }}
                disabled={activeLoading[row.original.AbonelikPlanID]}
              />
            </div>
          );
        },
        ...
        cell: ({ row }) => {
          const handleEditOrReload = async () => {
            if (!row.original.IsDeleted) {
              updateModalFunc(row.original.AbonelikPlanID)
            } else {
              alertModalFunc({
                title: "Geri Yüklensinmi?",
                text: "Abonelik planını geri yüklemek istiyormusunuz",
                actionButton: {
                  onClick: () => handleReloadOrDeleteFunc(row.original.AbonelikPlanID, "reload"),
                  text: "GERİ YÜKLE"
                },
                closeButton: {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const handleDelete = async () => {
            alertModalFunc({
              title: "Silinsin mi?",
              text: "Abonelik planını silmek istiyormusunuz",
              actionButton: {
                onClick: () => handleReloadOrDeleteFunc(row.original.AbonelikPlanID, "delete"),
                text: "SİL"
              },
              closeButton: {
                onClick: () => setOpenAlertModal(false),
  ...
  const isActiveFunc = async (itemId: number, value: boolean) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/abonelik-planlari/is-active-update`, {
        itemId: itemId,
        value: value
      });
      ...
      if (response?.status === 201) {
        setItemDataes(prevData =>
          prevData.map(item =>
            item.AbonelikPlanID === response.data.AbonelikPlanID ? response.data : item
          )
        );
      } else {
        toast.error('Plan durumu güncellenirken hata oluştu: ' + response.data?.message, { duration: 5000 });
      }
    } catch (error) {
      console.error(error);
      toast.error('Plan durumu güncellenirken hata oluştu', { duration: 5000 });
    } finally {
      setActiveLoading((prev) => ({
        ...prev,
        [itemId]: false,
  ...
  const handleReloadOrDeleteFunc = async (itemId: any, type: "delete" | "reload") => {
    ...
    try {
      const response = await axios.post(`${API_URL}/abonelik-planlari/${type}`, { itemId });
      ...
      if (response.status === 201) {
        ...
        toast.error("İşlem yapılırken hata oluştu AbonelikPlanlari");
      }
    } catch (error) {
      console.error(error);
      toast.error("İşlem yapılırken hata oluştu AbonelikPlanlari");
  ...
  const updateModalFunc = useCallback(async (selectId: any) => {
    ...
    if (!itemDataes || itemDataes.length === 0) {
      toast(`Bağlantı hatası`, {
        description: `AbonelikPlanlari Verileri mevcut değil`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    if (!selectId) {
      toast(`Bağlantı hatası`, {
        description: `Geçersiz AbonelikPlanID`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    const findTemplate = itemDataes.find((item) => item.AbonelikPlanID === selectId);
    if (findTemplate) {
      setSelectedItem(findTemplate);
      setOpenUpdateModal(true);
    } else {
      toast(`Bağlantı hatası`, {
        description: `AbonelikPlanlari bulunamadı`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
  ...
  const handleOpenModal = async (data: IModalResponse | 'close') => {
    ...
    try {
      if (data.status === 201 && data.data) {
        setItemDataes(prevData => {
          const existingItem = prevData.find(item => item.AbonelikPlanID === data.data.AbonelikPlanID);

          if (existingItem) {
            return prevData.map(item =>
              item.AbonelikPlanID === data.data.AbonelikPlanID ? data.data : item
            );
          } else {
        ...
        toast.error('Öğeyi oluştururken hata oluştu AbonelikPlanlari', { duration: 5000 });
      }
    } catch (error) {
  ...
  const Toolbar = ({ setSearchQuery }: { setSearchQuery: (query: string) => void }) => {
    ...
    return (
      <div className="card-header px-5 py-5 border-b-0 flex-wrap gap-2">
        <h3 className="card-title">Abonelik Planı</h3>
        <div className="input input-sm max-w-48">
          <KeenIcon icon="magnifier" />
          <input
            type="text"
            placeholder="Abonelik Planı Ara"
            value={inputValue}
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <a onClick={() => setOpenModal(true)} id="select_ip_btn" className="btn btn-sm btn-primary">Abonelik Planı Ekle</a>
          ...
          />}
          <AbonelikPlanlariCreateModal open={openModal} onOpenChange={handleOpenModal} />
          <AbonelikPlanlariUpdateModal open={openUpdateModal} onOpenChange={handleOpenModal} item={selectedItem} />
