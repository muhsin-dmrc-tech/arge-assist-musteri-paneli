...
import { TeknokentCreateModal, TeknokentUpdateModal } from '@/partials/modals/teknokentler';
...
const Teknokent = () => {
  ...
  const fetchBusinessTypess = async (queryParams: any) => {
    try {
      const response = await axios.get(
        `${API_URL}/teknokentler/get-teknokentler?${queryParams.toString()}`
      );
      return {
  ...
  const columns = useMemo<ColumnDef<IITemsTypesData>[]>(
    () => [
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.TeknokentAdi,
        id: 'TeknokentAdi',
        header: ({ column }) => <DataGridColumnHeader title="Dönem Adı" filter={<ColumnInputFilter column={column} />} column={column} />,
        enableSorting: true,
        cell: ({ row }) => {
          return (
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.Sehir,
        id: 'Sehir',
        header: ({ column }) => <DataGridColumnHeader title="Şehir" column={column} />,
        enableSorting: false,
        cell: ({ row }) => {
          return (
            <div className="flex items-center gap-4">
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.Ilce,
        id: 'Ilce',
        header: ({ column }) => <DataGridColumnHeader title="İlçe" column={column} />,
        enableSorting: false,
        cell: ({ row }) => {
          return (
            <div className="flex items-center gap-4">
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.OwnerKullanici,
        id: 'OwnerKullanici',
        ...
        cell: ({ row }) => {
          return (
            <div className="flex items-center gap-4">
              <div className="flex flex-col gap-0.5">
                <span className="leading-none font-medium text-sm text-green-500">
                  {row.original.OwnerKullanici?.AdSoyad?.split(' ')
                    .map(word => word.charAt(0).toUpperCase() + word.slice(1))
                    .join(' ') || <span className="leading-none font-medium text-sm text-red-500">Atanmadı</span>}
                </span>
              </div>
            </div>
          );
        ...
        cell: ({ row }) => {
          const handleEditOrReload = async () => {
            if (!row.original.IsDeleted) {
              updateModalFunc(row.original.TeknokentID)
            } else {
              alertModalFunc({
                title: "Geri Yüklensinmi?",
                text: "Teknokenti geri yüklemek istiyormusunuz",
                actionButton: {
                  onClick: () => handleReloadOrDeleteFunc(row.original.TeknokentID, "reload"),
                  text: "GERİ YÜKLE"
                },
                closeButton: {
          ...
          return activeLoading[row.original.TeknokentID] ? (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const handleDelete = async () => {
            alertModalFunc({
              title: "Silinsin mi?",
              text: "Teknokenti silmek istiyormusunuz",
              actionButton: {
                onClick: () => handleReloadOrDeleteFunc(row.original.TeknokentID, "delete"),
                text: "SİL"
              },
              closeButton: {
                onClick: () => setOpenAlertModal(false),
  ...
  const handleReloadOrDeleteFunc = async (itemId: any, type: "delete" | "reload") => {
    ...
    try {
      const response = await axios.post(`${API_URL}/teknokentler/${type}`, { itemId });
      if (response.status === 201) {
        // Başarılı olursa state'i güncelle
        setItemDataes(prevData =>
          prevData.map(item =>
            item.TeknokentID === response.data.TeknokentID ? response.data : item
          )
        );
        //window.location.reload()
      } else {
        toast.error("İşlem yapılırken hata oluştu Teknokent");
      }
    } catch (error) {
      console.error(error);
      toast.error("İşlem yapılırken hata oluştu Teknokent");
  ...
  const updateModalFunc = useCallback(async (selectId: any) => {
    ...
    if (!itemDataes || itemDataes.length === 0) {
      toast(`Bağlantı hatası`, {
        description: `Teknokent Verileri mevcut değil`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    if (!selectId) {
      toast(`Bağlantı hatası`, {
        description: `Geçersiz TeknokentID`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    const findTemplate = itemDataes.find((item) => item.TeknokentID === selectId);
    if (findTemplate) {
      setSelectedItem(findTemplate);
      setOpenUpdateModal(true);
    } else {
      toast(`Bağlantı hatası`, {
        description: `Teknokent bulunamadı`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
  ...
  const handleOpenModal = async (data: IModalResponse | 'close') => {
    ...
    try {
      if (data.status === 201 && data.data) {
        setItemDataes(prevData => {
          const existingItem = prevData.find(item => item.TeknokentID === data.data.TeknokentID);

          if (existingItem) {
            return prevData.map(item =>
              item.TeknokentID === data.data.TeknokentID ? data.data : item
            );
          } else {
        ...
        toast.error('Öğeyi oluştururken hata oluştu Teknokent', { duration: 5000 });
      }
    } catch (error) {
  ...
  const Toolbar = ({ setSearchQuery }: { setSearchQuery: (query: string) => void }) => {
    ...
    return (
      <div className="card-header px-5 py-5 border-b-0 flex-wrap gap-2">
        <h3 className="card-title">Teknokentler</h3>
        <div className="input input-sm max-w-48">
          <KeenIcon icon="magnifier" />
          <input
            type="text"
            placeholder="Teknokent ara"
            value={inputValue}
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <a onClick={() => setOpenModal(true)} id="select_ip_btn" className="btn btn-sm btn-primary">Teknokent Ekle</a>
          ...
          />}
          <TeknokentCreateModal open={openModal} onOpenChange={handleOpenModal} />
          <TeknokentUpdateModal open={openUpdateModal} onOpenChange={handleOpenModal} item={selectedItem} />
