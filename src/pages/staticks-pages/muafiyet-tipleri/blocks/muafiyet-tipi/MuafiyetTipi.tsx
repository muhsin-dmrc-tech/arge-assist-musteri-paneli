...
import { MuafiyetTipiCreateModal, MuafiyetTipiUpdateModal } from '@/partials/modals/muafiyet-tipleri';
...
const MuafiyetTipi = () => {
  ...
  const fetchBusinessTypess = async (queryParams: any) => {
    try {
      const response = await axios.get(
        `${API_URL}/muafiyet-tipleri/get-muafiyet-tipleri?${queryParams.toString()}`
      );
        ...
        cell: ({ row }) => {
          const handleEditOrReload = async () => {
            if (!row.original.IsDeleted) {
              updateModalFunc(row.original.MuafiyetTipiID)
            } else {
              alertModalFunc({
                title: "Geri Yüklensinmi?",
                text: "Muafiyet tipini geri yüklemek istiyormusunuz",
                actionButton: {
                  onClick: () => handleReloadOrDeleteFunc(row.original.MuafiyetTipiID, "reload"),
                  text: "GERİ YÜKLE"
                },
                closeButton: {
          ...
          return activeLoading[row.original.MuafiyetTipiID] ? (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const handleDelete = async () => {
            alertModalFunc({
              title: "Silinsin mi?",
              text: "Muafiyet Tipini silmek istiyormusunuz",
              actionButton: {
                onClick: () => handleReloadOrDeleteFunc(row.original.MuafiyetTipiID, "delete"),
                text: "SİL"
              },
              closeButton: {
                onClick: () => setOpenAlertModal(false),
  ...
  const handleReloadOrDeleteFunc = async (itemId: any, type: "delete" | "reload") => {
    ...
    try {
      const response = await axios.post(`${API_URL}/muafiyet-tipleri/${type}`, { itemId });
      if (response.status === 201) {
        // Başarılı olursa state'i güncelle
        setItemDataes(prevData =>
          prevData.map(item =>
            item.MuafiyetTipiID === response.data.MuafiyetTipiID ? response.data : item
          )
        );
        //window.location.reload()
      } else {
        toast.error("İşlem yapılırken hata oluştu MuafiyetTipi");
      }
    } catch (error) {
      console.error(error);
      toast.error("İşlem yapılırken hata oluştu MuafiyetTipi");
  ...
  const updateModalFunc = useCallback(async (selectId: any) => {
    ...
    if (!itemDataes || itemDataes.length === 0) {
      toast(`Bağlantı hatası`, {
        description: `MuafiyetTipi Verileri mevcut değil`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    if (!selectId) {
      toast(`Bağlantı hatası`, {
        description: `Geçersiz MuafiyetTipiID`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    const findTemplate = itemDataes.find((item) => item.MuafiyetTipiID === selectId);
    if (findTemplate) {
      setSelectedItem(findTemplate);
      setOpenUpdateModal(true);
    } else {
      toast(`Bağlantı hatası`, {
        description: `MuafiyetTipi bulunamadı`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
  ...
  const handleOpenModal = async (data: IModalResponse | 'close') => {
    ...
    try {
      if (data.status === 201 && data.data) {
        setItemDataes(prevData => {
          const existingItem = prevData.find(item => item.MuafiyetTipiID === data.data.MuafiyetTipiID);

          if (existingItem) {
            return prevData.map(item =>
              item.MuafiyetTipiID === data.data.MuafiyetTipiID ? data.data : item
            );
          } else {
        ...
        toast.error('Öğeyi oluştururken hata oluştu MuafiyetTipi', { duration: 5000 });
      }
    } catch (error) {
  ...
  const Toolbar = ({ setSearchQuery }: { setSearchQuery: (query: string) => void }) => {
    ...
    return (
      <div className="card-header px-5 py-5 border-b-0 flex-wrap gap-2">
        <h3 className="card-title">Muafiyet Tipi</h3>
        <div className="input input-sm max-w-48">
          <KeenIcon icon="magnifier" />
          <input
            type="text"
            placeholder="Muafiyet Tipi Ara"
            value={inputValue}
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <a onClick={() => setOpenModal(true)} id="select_ip_btn" className="btn btn-sm btn-primary">Muafiyet Tipi Ekle</a>
          ...
          />}
          <MuafiyetTipiCreateModal open={openModal} onOpenChange={handleOpenModal} />
          <MuafiyetTipiUpdateModal open={openUpdateModal} onOpenChange={handleOpenModal} item={selectedItem} />
