...
const Sozlesmeler = () => {
  ...
  const fetchBusinessTypess = async (queryParams: any) => {
    try {
      const response = await axios.get(
        `${API_URL}/sozlesmeler/get-sozlesmeler?${queryParams.toString()}`
      );
  ...
  const columns = useMemo<ColumnDef<IITemsTypesData>[]>(
    () => [
      ...
      },
      {
        ...
        id: 'Anahtar',
        header: ({ column }) => <DataGridColumnHeader title="Anahtar" filter={<ColumnInputFilter column={column} />} column={column} />,
        enableSorting: true,
        cell: ({ row }) => {
          return (
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.Baslik,
        id: 'Baslik',
        ...
        cell: ({ row }) => {
          const handleEditOrReload = async () => {
            if (!row.original.IsDeleted) {
              navigate(`/statick/sozlesme-upload/${row.original.Anahtar}`)
            } else {
              alertModalFunc({
                title: "Geri Yüklensinmi?",
                text: "Sözleşmeyi geri yüklemek istiyormusunuz",
                actionButton: {
                  onClick: () => handleReloadOrDeleteFunc(row.original.SozlesmeID, "reload"),
                  text: "GERİ YÜKLE"
                },
                closeButton: {
          ...
          return activeLoading[row.original.SozlesmeID] ? (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const handleDelete = async () => {
            alertModalFunc({
              title: "Silinsin mi?",
              text: "Sözleşmeyi silmek istiyormusunuz",
              actionButton: {
                onClick: () => handleReloadOrDeleteFunc(row.original.SozlesmeID, "delete"),
                text: "SİL"
              },
              closeButton: {
                onClick: () => setOpenAlertModal(false),
  ...
  const handleReloadOrDeleteFunc = async (itemId: any, type: "delete" | "reload") => {
    ...
    try {
      const response = await axios.post(`${API_URL}/sozlesmeler/${type}`, { itemId });
      if (response.status === 201) {
        // Başarılı olursa state'i güncelle
        setItemDataes(prevData =>
          prevData.map(item =>
            item.SozlesmeID === response.data.SozlesmeID ? response.data : item
          )
        );
        //window.location.reload()
      } else {
        toast.error("İşlem yapılırken hata oluştu Sozlesmeler");
      }
    } catch (error) {
      console.error(error);
      toast.error("İşlem yapılırken hata oluştu Sozlesmeler");
  ...
  const Toolbar = ({ setSearchQuery }: { setSearchQuery: (query: string) => void }) => {
    ...
    return (
      <div className="card-header px-5 py-5 border-b-0 flex-wrap gap-2">
        <h3 className="card-title">Sözleşmeler</h3>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <Link to='/statick/sozlesme-upload' id="select_ip_btn" className="btn btn-sm btn-primary">Sözleşme oluştur</Link>
