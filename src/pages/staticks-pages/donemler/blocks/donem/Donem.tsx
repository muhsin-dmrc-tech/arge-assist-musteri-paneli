...
import { DonemCreateModal, DonemUpdateModal } from '@/partials/modals/donemler';
...
const Donem = () => {
  ...
  const fetchBusinessTypess = async (queryParams: any) => {
    try {
      const response = await axios.get(
        `${API_URL}/donem/get-donemler?${queryParams.toString()}`
      );
  ...
  const columns = useMemo<ColumnDef<IITemsTypesData>[]>(
    () => [
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.DonemAdi,
        id: 'DonemAdi',
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.Ay,
        id: 'Ay',
        header: ({ column }) => <DataGridColumnHeader title="Ay" filter={<ColumnInputFilter column={column} />} column={column} />,
        enableSorting: true,
        cell: ({ row }) => {
          return (
            <div className="flex items-center gap-4">
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.Yil,
        id: 'Yil',
        header: ({ column }) => <DataGridColumnHeader title="Yıl" filter={<ColumnInputFilter column={column} />} column={column} />,
        enableSorting: true,
        cell: ({ row }) => {
          return (
            <div className="flex items-center gap-4">
        ...
        cell: ({ row }) => {
          const handleEditOrReload = async () => {
            if (!row.original.IsDeleted) {
              updateModalFunc(row.original.DonemID)
            } else {
              alertModalFunc({
                title: "Geri Yüklensinmi?",
                text: "Dönemi geri yüklemek istiyormusunuz",
                actionButton: {
                  onClick: () => handleReloadOrDeleteFunc(row.original.DonemID, "reload"),
                  text: "GERİ YÜKLE"
                },
                closeButton: {
          ...
          return activeLoading[row.original.DonemID] ? (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const handleDelete = async () => {
            alertModalFunc({
              title: "Silinsin mi?",
              text: "Dönemi silmek istiyormusunuz",
              actionButton: {
                onClick: () => handleReloadOrDeleteFunc(row.original.DonemID, "delete"),
                text: "SİL"
              },
              closeButton: {
                onClick: () => setOpenAlertModal(false),
  ...
  const handleReloadOrDeleteFunc = async (itemId: any, type: "delete" | "reload") => {
    ...
    try {
      const response = await axios.post(`${API_URL}/donem/${type}`, { itemId });
      if (response.status === 201) {
        // Başarılı olursa state'i güncelle
        setItemDataes(prevData =>
          prevData.map(item =>
            item.DonemID === response.data.DonemID ? response.data : item
          )
        );
        //window.location.reload()
      } else {
        toast.error("İşlem yapılırken hata oluştu Donem");
      }
    } catch (error) {
      console.error(error);
      toast.error("İşlem yapılırken hata oluştu Donem");
  ...
  const updateModalFunc = useCallback(async (selectId: any) => {
    ...
    if (!itemDataes || itemDataes.length === 0) {
      toast(`Bağlantı hatası`, {
        description: `Donem Verileri mevcut değil`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    const findTemplate = itemDataes.find((item) => item.DonemID === selectId);
    if (findTemplate) {
      setSelectedItem(findTemplate);
      setOpenUpdateModal(true);
    } else {
      toast(`Bağlantı hatası`, {
        description: `Donem bulunamadı`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
  ...
  const handleOpenModal = async (data: IModalResponse | 'close') => {
    ...
    try {
      if (data.status === 201 && data.data) {
        setItemDataes(prevData => {
          const existingItem = prevData.find(item => item.DonemID === data.data.DonemID);

          if (existingItem) {
            return prevData.map(item =>
              item.DonemID === data.data.DonemID ? data.data : item
            );
          } else {
        ...
        toast.error('Öğeyi oluştururken hata oluştu Donem', { duration: 5000 });
      }
    } catch (error) {
  ...
  const Toolbar = ({ setSearchQuery }: { setSearchQuery: (query: string) => void }) => {
    ...
    return (
      <div className="card-header px-5 py-5 border-b-0 flex-wrap gap-2">
        <h3 className="card-title">Dönem</h3>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <a onClick={() => setOpenModal(true)} id="select_ip_btn" className="btn btn-sm btn-primary">Dönem Ekle</a>
          ...
          />}
          <DonemCreateModal open={openModal} onOpenChange={handleOpenModal} />
          <DonemUpdateModal open={openUpdateModal} onOpenChange={handleOpenModal} item={selectedItem} />
