...
import { GorevlendirmeTuruCreateModal, GorevlendirmeTuruUpdateModal } from '@/partials/modals/gorevlendirme-turleri';
...
const GorevlendirmeTuru = () => {
  ...
  const fetchBusinessTypess = async (queryParams: any) => {
    try {
      const response = await axios.get(
        `${API_URL}/gorevlendirme-turu/get-gorevlendirme-turleri?${queryParams.toString()}`
      );
        ...
        cell: ({ row }) => {
          const handleEditOrReload = async () => {
            if (!row.original.IsDeleted) {
              updateModalFunc(row.original.GorevlendirmeTuruID)
            } else {
              alertModalFunc({
                title: "Geri Yüklensinmi?",
                text: "Görevlendirme türünü geri yüklemek istiyormusunuz",
                actionButton: {
                  onClick: () => handleReloadOrDeleteFunc(row.original.GorevlendirmeTuruID, "reload"),
                  text: "GERİ YÜKLE"
                },
                closeButton: {
          ...
          return activeLoading[row.original.GorevlendirmeTuruID] ? (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const handleDelete = async () => {
            alertModalFunc({
              title: "Silinsin mi?",
              text: "Görevlendirme türünü silmek istiyormusunuz",
              actionButton: {
                onClick: () => handleReloadOrDeleteFunc(row.original.GorevlendirmeTuruID, "delete"),
                text: "SİL"
              },
              closeButton: {
                onClick: () => setOpenAlertModal(false),
  ...
  const handleReloadOrDeleteFunc = async (itemId: any, type: "delete" | "reload") => {
    ...
    try {
      const response = await axios.post(`${API_URL}/gorevlendirme-turu/${type}`, { itemId });
      if (response.status === 201) {
        // Başarılı olursa state'i güncelle
        setItemDataes(prevData =>
          prevData.map(item =>
            item.GorevlendirmeTuruID === response.data.GorevlendirmeTuruID ? response.data : item
          )
        );
        //window.location.reload()
      } else {
        toast.error("İşlem yapılırken hata oluştu GorevlendirmeTuru");
      }
    } catch (error) {
      console.error(error);
      toast.error("İşlem yapılırken hata oluştu GorevlendirmeTuru");
  ...
  const updateModalFunc = useCallback(async (selectId: any) => {
    ...
    if (!itemDataes || itemDataes.length === 0) {
      toast(`Bağlantı hatası`, {
        description: `GorevlendirmeTuru Verileri mevcut değil`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    if (!selectId) {
      toast(`Bağlantı hatası`, {
        description: `Geçersiz GorevlendirmeTuruID`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    const findTemplate = itemDataes.find((item) => item.GorevlendirmeTuruID === selectId);
    if (findTemplate) {
      setSelectedItem(findTemplate);
      setOpenUpdateModal(true);
    } else {
      toast(`Bağlantı hatası`, {
        description: `GorevlendirmeTuru bulunamadı`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
  ...
  const handleOpenModal = async (data: IModalResponse | 'close') => {
    ...
    try {
      if (data.status === 201 && data.data) {
        setItemDataes(prevData => {
          const existingItem = prevData.find(item => item.GorevlendirmeTuruID === data.data.GorevlendirmeTuruID);

          if (existingItem) {
            return prevData.map(item =>
              item.GorevlendirmeTuruID === data.data.GorevlendirmeTuruID ? data.data : item
            );
          } else {
        ...
        toast.error('Öğeyi oluştururken hata oluştu GorevlendirmeTuru', { duration: 5000 });
      }
    } catch (error) {
  ...
  const Toolbar = ({ setSearchQuery }: { setSearchQuery: (query: string) => void }) => {
    ...
    return (
      <div className="card-header px-5 py-5 border-b-0 flex-wrap gap-2">
        <h3 className="card-title">Görevlendirme Türü</h3>
        <div className="input input-sm max-w-48">
          <KeenIcon icon="magnifier" />
          <input
            type="text"
            placeholder="Görevlendirme türü ara"
            value={inputValue}
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <a onClick={() => setOpenModal(true)} id="select_ip_btn" className="btn btn-sm btn-primary">Görevlendirme Türü Ekle</a>
          ...
          />}
          <GorevlendirmeTuruCreateModal open={openModal} onOpenChange={handleOpenModal} />
          <GorevlendirmeTuruUpdateModal open={openUpdateModal} onOpenChange={handleOpenModal} item={selectedItem} />
