...
import { GiderTipiCreateModal, GiderTipiUpdateModal } from '@/partials/modals/gider-tipleri';
...
const GiderTipi = () => {
  ...
  const fetchBusinessTypess = async (queryParams: any) => {
    try {
      const response = await axios.get(
        `${API_URL}/gider-tipleri/get-gider-tipleri?${queryParams.toString()}`
      );
        ...
        cell: ({ row }) => {
          const handleEditOrReload = async () => {
            if (!row.original.IsDeleted) {
              updateModalFunc(row.original.GiderTipiID)
            } else {
              alertModalFunc({
                title: "Geri Yüklensinmi?",
                text: "Gider Tipini geri yüklemek istiyormusunuz",
                actionButton: {
                  onClick: () => handleReloadOrDeleteFunc(row.original.GiderTipiID, "reload"),
                  text: "GERİ YÜKLE"
                },
                closeButton: {
          ...
          return activeLoading[row.original.GiderTipiID] ? (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const handleDelete = async () => {
            alertModalFunc({
              title: "Silinsin mi?",
              text: "Gider Tipini silmek istiyormusunuz",
              actionButton: {
                onClick: () => handleReloadOrDeleteFunc(row.original.GiderTipiID, "delete"),
                text: "SİL"
              },
              closeButton: {
                onClick: () => setOpenAlertModal(false),
  ...
  const handleReloadOrDeleteFunc = async (itemId: any, type: "delete" | "reload") => {
    ...
    try {
      const response = await axios.post(`${API_URL}/gider-tipleri/${type}`, { itemId });
      if (response.status === 201) {
        // Başarılı olursa state'i güncelle
        setItemDataes(prevData =>
          prevData.map(item =>
            item.GiderTipiID === response.data.GiderTipiID ? response.data : item
          )
        );
        //window.location.reload()
      } else {
        toast.error("İşlem yapılırken hata oluştu GiderTipi");
      }
    } catch (error) {
      console.error(error);
      toast.error("İşlem yapılırken hata oluştu GiderTipi");
  ...
  const updateModalFunc = useCallback(async (selectId: any) => {
    ...
    if (!itemDataes || itemDataes.length === 0) {
      toast(`Bağlantı hatası`, {
        description: `GiderTipi Verileri mevcut değil`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    if (!selectId) {
      toast(`Bağlantı hatası`, {
        description: `Geçersiz GiderTipiID`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    const findTemplate = itemDataes.find((item) => item.GiderTipiID === selectId);
    if (findTemplate) {
      setSelectedItem(findTemplate);
      setOpenUpdateModal(true);
    } else {
      toast(`Bağlantı hatası`, {
        description: `GiderTipi bulunamadı`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
  ...
  const handleOpenModal = async (data: IModalResponse | 'close') => {
    ...
    try {
      if (data.status === 201 && data.data) {
        setItemDataes(prevData => {
          const existingItem = prevData.find(item => item.GiderTipiID === data.data.GiderTipiID);

          if (existingItem) {
            return prevData.map(item =>
              item.GiderTipiID === data.data.GiderTipiID ? data.data : item
            );
          } else {
        ...
        toast.error('Öğeyi oluştururken hata oluştu GiderTipi', { duration: 5000 });
      }
    } catch (error) {
  ...
  const Toolbar = ({ setSearchQuery }: { setSearchQuery: (query: string) => void }) => {
    ...
    return (
      <div className="card-header px-5 py-5 border-b-0 flex-wrap gap-2">
        <h3 className="card-title">Gider Tipi</h3>
        <div className="input input-sm max-w-48">
          <KeenIcon icon="magnifier" />
          <input
            type="text"
            placeholder="Gider Tipi Ara"
            value={inputValue}
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <a onClick={() => setOpenModal(true)} id="select_ip_btn" className="btn btn-sm btn-primary">Gider Tipi Ekle</a>
          ...
          />}
          <GiderTipiCreateModal open={openModal} onOpenChange={handleOpenModal} />
          <GiderTipiUpdateModal open={openUpdateModal} onOpenChange={handleOpenModal} item={selectedItem} />
