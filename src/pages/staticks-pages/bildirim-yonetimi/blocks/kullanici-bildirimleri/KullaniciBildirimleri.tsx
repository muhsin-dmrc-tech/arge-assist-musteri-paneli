...
const KullaniciBildirimleri = () => {
  ...
  const fetchBildirimler = async (queryParams: any) => {
    try {
      const response = await axios.get(
        `${API_URL}/kullanici-bildirimleri/get-bildirimler?${queryParams.toString()}`
      );
  ...
  const columns = useMemo<ColumnDef<ITemplateData>[]>(
    () => [
      ...
      {
        accessorFn: (row: ITemplateData) => row.KullaniciID,
        id: 'KullaniciID',
        header: ({ column }) => <DataGridColumnHeader title="Kullanıcı ID" filter={<ColumnInputFilter column={column} />} column={column} />,
        enableSorting: true,
        cell: ({ row }) => {
          return (
      ...
      },
      {
        ...
        enableSorting: true,
        cell: (info) => {
          return  info.row.original.Icerik.length > 50 ? info.row.original.Icerik.slice(0,50)+'...' : info.row.original.Icerik;
        },
        meta: {
          headerClassName: 'min-w-[50px]',
          cellClassName: 'text-sm text-gray-800 font-normal'
      ...
      {
        accessorFn: (row) => row.OkunduMu,
        id: 'OkunduMu',
        header: ({ column }) => <DataGridColumnHeader title="OkunduMu" column={column} />,
        enableSorting: false,
        cell: (info) => {
          return <div className={`flex flex-col ${info.row.original.OkunduMu ? 'text-green-300' : 'text-red-300'} text-center`}>
          <span className="text text-sm">{info.row.original.OkunduMu ? 'Okundu' : 'Okunmadı'}</span>
          {info.row.original.OkunmaTarihi && <span className="text text-sm italic">{formatDate(info.row.original.OkunmaTarihi)}</span>}
        </div>;
        },
        meta: {
          headerClassName: 'min-w-[50px]',
  ...
  const Toolbar = ({ setSearchQuery }: { setSearchQuery: (query: string) => void }) => {
    ...
    return (
      <div className="card-header px-5 py-5 border-b-0 flex-wrap">
        <h3 className="card-title">Kullanıcı Bildirimleri</h3>
