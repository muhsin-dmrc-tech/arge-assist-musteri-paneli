...
import { CalismaTuruCreateModal, CalismaTuruUpdateModal } from '@/partials/modals/calisma-turleri';
...
const CalismaTuru = () => {
  ...
  const fetchBusinessTypess = async (queryParams: any) => {
    try {
      const response = await axios.get(
        `${API_URL}/calisma-turu/get-calisma-turleri?${queryParams.toString()}`
      );
        ...
        cell: ({ row }) => {
          const handleEditOrReload = async () => {
            if (!row.original.IsDeleted) {
              updateModalFunc(row.original.CalismaTuruID)
            } else {
              alertModalFunc({
                title: "Geri Yüklensinmi?",
                text: "Çalışma türünü geri yüklemek istiyormusunuz",
                actionButton: {
                  onClick: () => handleReloadOrDeleteFunc(row.original.CalismaTuruID, "reload"),
                  text: "GERİ YÜKLE"
                },
                closeButton: {
          ...
          return activeLoading[row.original.CalismaTuruID] ? (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const handleDelete = async () => {
            alertModalFunc({
              title: "Silinsin mi?",
              text: "Çalışma türünü silmek istiyormusunuz",
              actionButton: {
                onClick: () => handleReloadOrDeleteFunc(row.original.CalismaTuruID, "delete"),
                text: "SİL"
              },
              closeButton: {
                onClick: () => setOpenAlertModal(false),
  ...
  const handleReloadOrDeleteFunc = async (itemId: any, type: "delete" | "reload") => {
    ...
    try {
      const response = await axios.post(`${API_URL}/calisma-turu/${type}`, { itemId });
      if (response.status === 201) {
        // Başarılı olursa state'i güncelle
        setItemDataes(prevData =>
          prevData.map(item =>
            item.CalismaTuruID === response.data.CalismaTuruID ? response.data : item
          )
        );
        //window.location.reload()
      } else {
        toast.error("İşlem yapılırken hata oluştu CalismaTuru");
      }
    } catch (error) {
      console.error(error);
      toast.error("İşlem yapılırken hata oluştu CalismaTuru");
  ...
  const updateModalFunc = useCallback(async (selectId: any) => {
    ...
    if (!itemDataes || itemDataes.length === 0) {
      toast(`Bağlantı hatası`, {
        description: `CalismaTuru Verileri mevcut değil`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    if (!selectId) {
      toast(`Bağlantı hatası`, {
        description: `Geçersiz CalismaTuruID`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    const findTemplate = itemDataes.find((item) => item.CalismaTuruID === selectId);
    if (findTemplate) {
      setSelectedItem(findTemplate);
      setOpenUpdateModal(true);
    } else {
      toast(`Bağlantı hatası`, {
        description: `CalismaTuru bulunamadı`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
  ...
  const handleOpenModal = async (data: IModalResponse | 'close') => {
    ...
    try {
      if (data.status === 201 && data.data) {
        setItemDataes(prevData => {
          const existingItem = prevData.find(item => item.CalismaTuruID === data.data.CalismaTuruID);

          if (existingItem) {
            return prevData.map(item =>
              item.CalismaTuruID === data.data.CalismaTuruID ? data.data : item
            );
          } else {
        ...
        toast.error('Öğeyi oluştururken hata oluştu CalismaTuru', { duration: 5000 });
      }
    } catch (error) {
  ...
  const Toolbar = ({ setSearchQuery }: { setSearchQuery: (query: string) => void }) => {
    ...
    return (
      <div className="card-header px-5 py-5 border-b-0 flex-wrap gap-2">
        <h3 className="card-title">Çalisma Türü</h3>
        <div className="input input-sm max-w-48">
          <KeenIcon icon="magnifier" />
          <input
            type="text"
            placeholder="Calisma Turu Ara"
            value={inputValue}
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <a onClick={() => setOpenModal(true)} id="select_ip_btn" className="btn btn-sm btn-primary">Çalisma Türü Ekle</a>
          ...
          />}
          <CalismaTuruCreateModal open={openModal} onOpenChange={handleOpenModal} />
          <CalismaTuruUpdateModal open={openUpdateModal} onOpenChange={handleOpenModal} item={selectedItem} />
