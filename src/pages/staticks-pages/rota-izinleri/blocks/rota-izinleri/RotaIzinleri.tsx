...
import { RotaIzinleriCreateModal, RotaIzinleriUpdateModal } from '@/partials/modals/rota-izinleri';
...
const RotaIzinleri = () => {
  ...
  const fetchBusinessTypess = async (queryParams: any) => {
    try {
      const response = await axios.get(
        `${API_URL}/rota-izinleri/get-rota-izinler?${queryParams.toString()}`
      );
  ...
  const columns = useMemo<ColumnDef<IITemsTypesData>[]>(
    () => [
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.Anahtar,
        id: 'Anahtar',
        header: ({ column }) => <DataGridColumnHeader title="Anahtar" column={column} />,
        enableSorting: true,
        cell: ({ row }) => {
          return (
      ...
      },
      {
        ...
        id: 'Tanim',
        header: ({ column }) => <DataGridColumnHeader title="Tanım" column={column} />,
        enableSorting: true,
        cell: ({ row }) => {
          return (
            <div className="flex items-center gap-4">
      ...
      {
        accessorFn: (row: IITemsTypesData) => row.Type,
        id: 'Type',
          ...
          const handleEditOrReload = async () => {
            if (!row.original.IsDeleted) {
              ...
              alertModalFunc({
                ...
                text: "Rota iznini geri yüklemek istiyormusunuz",
                actionButton: {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const handleDelete = async () => {
            alertModalFunc({
              title: "Silinsin mi?",
              text: "Rota iznini silmek istiyormusunuz",
              actionButton: {
  ...
  const handleReloadOrDeleteFunc = async (itemId: any, type: "delete" | "reload") => {
    ...
    try {
      const response = await axios.post(`${API_URL}/rota-izinleri/${type}`, { itemId });
      if (response.status === 201) {
        ...
        toast.error("İşlem yapılırken hata oluştu RotaIzinleri");
      }
    } catch (error) {
      console.error(error);
      toast.error("İşlem yapılırken hata oluştu RotaIzinleri");
  ...
  const updateModalFunc = useCallback(async (selectId: any) => {
    ...
    if (!itemDataes || itemDataes.length === 0) {
      toast(`Bağlantı hatası`, {
        description: `RotaIzinleri Verileri mevcut değil`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    if (!selectId) {
      toast(`Bağlantı hatası`, {
        description: `Geçersiz RotaIzinleriID`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
    ...
    if (findTemplate) {
      ...
      toast(`Bağlantı hatası`, {
        description: `RotaIzinleri bulunamadı`,
        action: {
          label: 'Ok',
          onClick: () => { console.log('Ok') }
        }
  ...
  const handleOpenModal = async (data: IModalResponse | 'close') => {
    ...
    try {
      if (data.status === 201 && data.data) {
        ...
        toast.error('Öğeyi oluştururken hata oluştu RotaIzinleri', { duration: 5000 });
      }
    } catch (error) {
  ...
  const Toolbar = ({ setSearchQuery }: { setSearchQuery: (query: string) => void }) => {
    ...
    return (
      <div className="card-header px-5 py-5 border-b-0 flex-wrap gap-2">
        <h3 className="card-title">Rota İzinleri</h3>
        <div className="input input-sm max-w-48">
          <KeenIcon icon="magnifier" />
          <input
            type="text"
            placeholder="Rota izni ara"
            value={inputValue}
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <a onClick={() => setOpenModal(true)} id="select_ip_btn" className="btn btn-sm btn-primary">Rota izni Ekle</a>
          ...
          />}
          <RotaIzinleriCreateModal open={openModal} onOpenChange={handleOpenModal} />
          <RotaIzinleriUpdateModal open={openUpdateModal} onOpenChange={handleOpenModal} item={selectedItem} />
