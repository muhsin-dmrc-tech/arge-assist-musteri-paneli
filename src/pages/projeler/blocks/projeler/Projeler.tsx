...
const Projeler = () => {
  ...
  const fetchBusinessTypess = async (queryParams: any) => {
    try {
      const response = await axios.get(
        `${API_URL}/projeler/get-projeler?${queryParams.toString()}`
      );
        ...
        cell: ({ row }) => {
          const handleEditOrReload = async () => {
            if (!row.original.IsDeleted) {
              if (!yetkiKontrolu(seciliFirma ?? seciliTeknokent,currentUser, ['proje-edit'])) {
                toast.error("Bu işlemi yapma izniniz yok.");
                return
              }
              navigate(`/${seciliFirma?.FirmaID}/projeler/edit/${row.original.ProjeID}`)
            } else {
              alertModalFunc({
                title: "Geri Yüklensinmi?",
                text: "Projeyi geri yüklemek istiyormusunuz",
                actionButton: {
                  onClick: () => handleReloadOrDeleteFunc(row.original.ProjeID, "reload"),
                  text: "GERİ YÜKLE"
                },
                closeButton: {
          ...
          return activeLoading[row.original.ProjeID] ? (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const handleDelete = async () => {
            alertModalFunc({
              title: "Silinsin mi?",
              text: "Projeyi silmek istiyormusunuz",
              actionButton: {
                onClick: () => handleReloadOrDeleteFunc(row.original.ProjeID, "delete"),
                text: "SİL"
              },
              closeButton: {
                onClick: () => setOpenAlertModal(false),
  ...
  const handleReloadOrDeleteFunc = async (itemId: any, type: "delete" | "reload") => {
    ...
    try {
      const response = await axios.post(`${API_URL}/projeler/${type}`, { itemId });
      if (response.status === 201) {
        // Başarılı olursa state'i güncelle
        setItemDataes(prevData =>
          prevData.map(item =>
            item.ProjeID === response.data.ProjeID ? response.data : item
          )
        );
        //window.location.reload()
      } else {
        toast.error("İşlem yapılırken hata oluştu Projeler");
      }
    } catch (error) {
      console.error(error);
      toast.error("İşlem yapılırken hata oluştu Projeler");
      ...
      <div className="flex-col px-5 py-5 border-b-0">
        <div className="flex w-full justify-between flex-wrap gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
           {/*  {seciliFirma &&
              ['proje-seeing', 'proje-edit'].every(izin => seciliFirma?.Gurup?.Yetkiler.includes(izin)) || seciliFirma?.Rol === 'owner' &&
              <Link to={`/${seciliFirma?.FirmaID}/projeler/edit`} className="btn btn-sm btn-primary">Proje Ekle</Link>
            } */}
